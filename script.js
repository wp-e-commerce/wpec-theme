...
$(document).on('click', '.older', function() {
  ...
  setTimeout(function(){
    var $next = $('.current').removeClass('current').next('.group');

    if ($next.length) {
      $next.removeClass('fadeout').addClass('current fadein');
    } else {
      $('.group:first').removeClass('fadeout').addClass('current fadein');
    }
  }, 250);
...
$(document).on('click', '.newer', function() {
  ...
  setTimeout(function(){
    var $prev = $('.current').removeClass('current').prev('.group');

    if ($prev.length) {
      $prev.removeClass('fadeout').addClass('current fadein');
    } else {
      $('.group:last').removeClass('fadeout').addClass('current fadein');
    }
  }, 250);
